...
## 🤖 FR3 MuJoCo 블록 밀기 주요 결론

**평가 기준:** 오른쪽/중앙/왼쪽 접근을 보강한 동일한 balanced-140 데이터셋, batch size 8, 동일한 고정 초기조건에서 5회 평가. 성공 기준은 블록과 목표의 XY 거리 `0.035m` 이하이다.

| 구분 | 학습 step | 성공률 | 최종 목표 거리 평균 | 핵심 해석 |
|------|----------:|-------:|--------------------:|-----------|
| ACT | 5k | **100% (5/5)** | **0.0312m** | 접근 방향과 후반부 보정 시연을 균형 있게 추가하자 모든 rollout에서 목표에 진입 |
| Diffusion Policy | 5k | 20% (1/5) | 0.0778m | 측면 drift와 불안정한 접근이 주된 실패 원인 |
| Diffusion Policy | 10k | 40% (2/5) | 0.0578m | 학습량 증가로 성공률과 평균 목표 거리 개선 |
| Diffusion Policy | 20k | **80% (4/5)** | 0.0406m | 5k 성능 저하의 주원인이 학습 부족이었음을 지지 |
...
**주요 결론:** 기존 50개 시연은 성공 위치까지 도달했지만 접근 방향과 후반부 보정 궤적의 다양성이 부족해 ACT가 목표 앞에서 멈췄다. 기존 데이터에 오른쪽/중앙/왼쪽 접근을 각각 30개씩 추가한 뒤 ACT는 같은 5k 학습 예산에서 5/5 성공했다. 따라서 이 태스크에서는 단순 데이터 양보다 **일관된 성공 시연과 접근·복구 궤적의 균형**이 핵심이었다. DP는 학습 step을 늘리면서 20%에서 80%까지 개선되어 5k 비교만으로 알고리즘의 우열을 단정할 수 없다.

> 이 결과는 동일한 고정 초기조건에서 반복한 5회의 결정론적 평가이며, 무작위 초기조건에 대한 일반화 성능을 의미하지 않는다. 구현, 데이터 수집, 전체 비교 지표는 [FR3 블록 밀기 프로젝트 문서](projects/fr3_block_push/README.md)를 참고한다.
